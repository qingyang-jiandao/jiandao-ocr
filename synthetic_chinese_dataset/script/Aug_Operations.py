...
class Skew(Operation):
    ...
    def perform_operation(self, image, border_color=(255, 255, 255, 0)):
        """
        ...
        #print(perspective_skew_coefficients_matrix)
        transformed_img, points = image.transform((new_w, new_h),
                                            Image.PERSPECTIVE,
                                            perspective_skew_coefficients_matrix,
                                            resample=Image.BICUBIC), np.array(plane)

        # transformed_img = transformed_img.convert('RGBA')
        # transformed_img_arr = np.array(transformed_img).astype(np.uint32)
        # #channel alpha already got broken: mask = transformed_img_arr[:, :, 3]
        # mask = transformed_img_arr[:, :, 3]
        # #np.savetxt('skew_mask' + '.csv', mask, delimiter=',')
        # r = transformed_img_arr[:, :, 0]
        # g = transformed_img_arr[:, :, 1]
        # b = transformed_img_arr[:, :, 2]
        # mask_index = (r+g+b==0)
        # transformed_img_arr[mask_index] = (255, 255, 255, 255)
        # transformed_img_arr = transformed_img_arr.astype(np.uint8)
        # transformed_img = Image.fromarray(transformed_img_arr)


        transformed_img = transformed_img.convert('RGBA')
        transformed_img_arr = np.array(transformed_img).astype(np.uint32)
        x2y_list = [dict() for _ in range(len(plane))]
        for i in range(len(plane)):
            ...
            min_x = math.floor(min(a[0], b[0]))
            max_x = math.ceil(max(a[0], b[0]))
            min_y = math.floor(min(a[1], b[1]))
            max_y = math.ceil(max(a[1], b[1]))
            # print("min_x", min_x, "max_x", max_x)
            div = b[0] - a[0]
            if abs(b[0] - a[0]) < 0.00001:
                div = 0.0001
                # continue
        ...
        dirty_data = False
        lngaxis, lataxis = zip(*plane)
        init_minlat, init_maxlat = math.floor(min(lataxis)), math.ceil(max(lataxis))
        minlng, maxlng = math.floor(min(lngaxis)), math.ceil(max(lngaxis))
        # print(minlng, "lat", minlng)
        for x in range(minlng, maxlng):
            minlat = maxlat = -1
            for i in range(len(x2y_list)):
                if x2y_list[i].get(x) != None:
                    if minlat == -1:
                        minlat = x2y_list[i][x]
                        continue
                    if maxlat == -1:
                        maxlat = x2y_list[i][x]
                if minlat != -1 and maxlat != -1:
                    break
            if minlat > maxlat:
                xxx = minlat
                minlat = maxlat
                maxlat = xxx
            if minlat != -1 and maxlat != -1:
                # print(minlat, " lng ", maxlat)
                transformed_img_arr[0:minlat+1, x] = border_color
                transformed_img_arr[maxlat-1:init_maxlat, x] = border_color
...
class Shear(Operation):
    ...
    def perform_operation(self, image, border_color=(255, 255, 255, 0)):
        ...
        if direction == "x":
            ...
            transformed_img = image.transform((width, int(round(height + shift_in_pixels))),
                                    Image.AFFINE,
                                    transform_matrix,
                                    Image.BICUBIC)
            ...
            return transformed_img ,np.array(plane) #.resize((width, height), resample=Image.BICUBIC)
